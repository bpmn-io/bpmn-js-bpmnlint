...
Linting.prototype.activateLinting = function() {
  ...
  this.createIssues(this._issues);
...
Linting.prototype.update = function() {
  var newIssues = otherIssuesMock;

  newIssues = this.formatIssues(newIssues);
  ...
  for (var id in newIssues) {
    if (!this._issues[id]) {
      ...
      if (!isEqual(newIssues[id], this._issues[id])) {
  ...
  this.removeIssues(remove);
  this.createIssues(add);
